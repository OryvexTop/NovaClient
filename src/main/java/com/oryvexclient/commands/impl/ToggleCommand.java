...
public class ToggleCommand extends Command {
    ...
    public void execute(String[] args) {
        if (args.length < 2) { OryvexClient.getInstance().getCommandManager().sendMessage("Usage: .toggle <module>"); return; }
        Module m = OryvexClient.getInstance().getModuleManager().getModuleByName(args[1]);
        if (m == null) { OryvexClient.getInstance().getCommandManager().sendMessage("Module not found."); return; }
        m.toggle();
